...
class SelectType(BaseType):
    ...
    def contains(self, other_value):
        print("Contains :: Comparing self value in options...", self.value, other_value)
        for val in other_value:
            if self._case_insensitive_equal_to(val, self.value):
                return True
        return False

    @type_operator(FIELD_SELECT, assert_type_for_arguments=False)
    def does_not_contain(self, other_value):
        print("Doesnt Contains :: Comparing self value in options...", self.value, other_value)
        for val in other_value:
            if self._case_insensitive_equal_to(val, self.value):
                return False
        return True
